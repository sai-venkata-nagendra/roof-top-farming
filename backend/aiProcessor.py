...
def analyze_image(image_path):
    ...
    # Remove obstacles from sunlight zones to create clean recommended zones
    recommended_mask = cv2.bitwise_and(sunlight_mask, cv2.bitwise_not(edges))  # 🔹 Remove obstacles from recommended zones

    def get_positions(mask, height=0.1, step=50):  # 🔹 Limit points for efficiency
        positions = []
        h, w = mask.shape
    ...
    return {
        ...
        "recommended_zones": get_positions(recommended_mask, height=0.05)  # 🔹 Uses cleaned recommended zones
    }
